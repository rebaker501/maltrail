...
def fetch():
    ...
    uri = "/v2/indicators/filters=rating%3E3,threatAssessConfidence%3E90,?resultStart=&resultLimit=50"
    ...
    for i in range(50,total,50):
        uri = "/v2/indicators/filters=rating%3E3,threatAssessConfidence%3E90,?resultStart=&resultLimit=50".format(i);
        json = query(uri)
        allData += json["data"]["indicator"]
    ...
    for record in allData:
        if record["type"] == "Host" or record["type"] == "Address":
            output[record["summary"]] = ("ThreatConnect, Added: {}".format(record["dateAdded"]),record["webLink"])
